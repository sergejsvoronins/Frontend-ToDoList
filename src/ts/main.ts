...
let mainContainer = document.createElement("div") as HTMLDivElement;
let todoFormEl = document.createElement("form") as HTMLFormElement;
const toDoHeader = document.createElement("h1");
const userInputLabelEl = document.createElement("label") as HTMLLabelElement;
const userInputEl = document.createElement("input") as HTMLInputElement;
...
mainContainer.appendChild(todoFormEl);
todoFormEl.appendChild(userInputLabelEl);
todoFormEl.appendChild(userInputEl);
todoFormEl.appendChild(btnContainer);
...
function loadFromLS(){
    let toDoLS = JSON.parse(localStorage.getItem("toDoList") || "[]");
        ...
        toDoList = toDoLS.map((toDos: ToDoObject) => {
...
function deleteToDo (someList:ToDoObject[], index:number){
    someList.splice (index, 1);
}

function sortAz(){
    toDoList.sort((a:ToDoObject, b:ToDoObject)=>{
        let fa = a.status,
            fb = b.status;
            if (fa < fb) {
                return -1;
    ...
    toDoList.sort((a:ToDoObject, b:ToDoObject) : number | any => {
        let fa = a.toDo,
            fb = b.toDo;
...
let toDoList : ToDoObject []= [];
loadFromLS();
createHTML();
todoFormEl.addEventListener("submit", (e:Event)=>{
    e.preventDefault();
